...
local root = get_root()
local bundled = root .. "/bundled/blaze"
...
if npm_init_result.code ~= 0 then
    ...
    -- Install blaze
    status("Installing blaze...", vim.log.levels.INFO)
    ...
    if npm_install_result.code ~= 0 then
        error("Failed to light it up with blaze: " .. npm_install_result.stderr)
    else
        status("Light it up with blaze.nvim - responsibly ;)", vim.log.levels.INFO)
    end
end
