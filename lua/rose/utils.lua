...
-- Get API key either from environment variable or by prompting user.
---@param env_var string # Environment variable name to check for API key
---@param prompt_message string # The prompt message to show the user
---@return string|nil # The API key, or nil if not provided
M.get_api_key = function(env_var, prompt_message)
  local key = os.getenv(env_var)
  if not key or key == "" then
    key = vim.fn.input(prompt_message .. ": ")
    if key and key ~= "" then
...
---@return table # The prepared model request payload
M.prepare_payload = function(messages, model_name, params)
  ...
  for k, v in pairs(params) do
    if k == "temperature" then
      model_req[k] = tostring(math.max(0, math.min(2, v or 1))) -- Convert number to string
    elseif k == "top_p" then
      model_req[k] = tostring(math.max(0, math.min(1, v or 1))) -- Convert number to string
    else
      if type(v) == "table" then
        model_req[k] = {}
        for pk, pv in pairs(v) do
          model_req[k][pk] = pv -- Ensure the type matches here
        end
      elseif type(v) == "number" then
        model_req[k] = tostring(v) -- Convert number to string to avoid type mismatch
      elseif type(v) == "boolean" or type(v) == "string" then
        model_req[k] = v -- Assign directly if it’s a boolean or string
      else
        -- Log an error or warning if the type is unexpected
        print(
          string.format("Warning: Unsupported type for key %s. Expected boolean, string, or table, got %s", k, type(v))
        )
      end
    end
  end
