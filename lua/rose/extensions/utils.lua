...
function M.setup_codecompanion_variables(enabled)
    ...
    rose.on({ "servers_updated", "resource_list_changed" }, function(_)
        local hub = rose.get_hub_instance()
        if not hub then
            return
        end
        ...
        -- remove existing rose variables that start with rose
        for key, value in pairs(cc_variables) do
            local id = value.id or ""
            if id:sub(1, 3) == "rose" then
...
function M.setup_codecompanion_slash_commands(enabled)
    ...
    local rose = require("rose")
    --setup event listners to update variables, tools etc
    rose.on({ "servers_updated", "prompt_list_changed" }, function(_)
        local hub = rose.get_hub_instance()
        if not hub then
            return
        end
