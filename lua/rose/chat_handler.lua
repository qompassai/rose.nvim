...
function ChatHandler:context(params)
  ...
  local file_name = ""
  local buf = utils.get_buffer(".rose.md")
  if buf then
  ...
  local callback = function(command)
    ...
    if sys_prompt ~= "" then
      ...
      if repo_instructions ~= "" and sys_prompt ~= "" then
        -- append the repository instructions from .rose.md to the system prompt
