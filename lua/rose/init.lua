...
local prompts = require("rose.prompts")
local M = {}
...
local function create_window(cmd, opts)
    ...
    elseif display_mode == "horizontal-split" then
        vim.cmd("split rose.nvim")
        setup_split()
    else
        vim.cmd("vnew rose.nvim")
        setup_split()
    end
...
M.exec = function(options)
    ...
    if string.find(cmd, "%$body") then
        ...
        if opts.model_options ~= nil then -- override model options from rose command (if exist)
...
M.run_command = function(cmd, opts)
    ...
    local group = vim.api.nvim_create_augroup("rose", { clear = true })
...
vim.api.nvim_create_user_command("Rose", function(arg)
    local mode
    if arg.range == 0 then
        mode = "n"
    else
